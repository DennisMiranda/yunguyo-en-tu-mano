# Phase 7: Detalle del Emprendimiento

## Objective

Página de detalle completa del emprendimiento.
...
### 2. `src/pages/public/EmprendimientoDetalle.tsx`
...
export default function EmprendimientoDetalle() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      if (emp) {
        setEmprendimiento(emp as Emprendimiento);
        setImagenSeleccionada((emp as Emprendimiento).imagen_principal);
        ...
        const { data: rels } = await supabase
          ...
          .eq('categoria_id', (emp as Emprendimiento).categoria_id)
          .neq('id', emp.id)
          .limit(3);
...
### 4. Create `e2e/business-detail.spec.ts`
...
**Files:** src/lib/routes.ts, src/pages/public/EmprendimientoDetalle.tsx, src/App.tsx

### Commit 2: test(e2e): add business detail tests

**Files:** e2e/business-detail.spec.ts

### Commit 3: docs: add phase 7 plan

**Files:** docs/phases/phase-7-business-detail/plan.md, docs/phases/phase-7-business-detail/commit-plan.md
